...
from debate_kg.data.breastmnist import load_samples
from debate_kg.debate.breast_orchestrator import (
    run_opinion_debate,
    run_single_agent,
    run_structured_debate,
)
from debate_kg.eval.breast_metrics import (
    accuracy,
    auc_roc,
    convergence_rate,
    sensitivity,
...
from debate_kg.kg.loader import load_definitions, load_kg_from_json, serialize_kg_for_prompt
from debate_kg.kg.schema import DebateEdge, DebateNode, JudgeScore, SingleAgentResult
...
def main(cfg: DictConfig) -> None:
    ...
    # Build judge (not needed for single_agent).
    judge = None
    if cfg.run.mode != "single_agent":
        from debate_kg.judges.medgemma_judge import build_medgemma_judge
    ...
    for sample in samples:
        ...
        if mode == "single_agent":
            ...
            verdict, malignant_score, rounds = _run_mode_adaptive(sample, cfg, output_dir, judge, kg_text)
        else:
