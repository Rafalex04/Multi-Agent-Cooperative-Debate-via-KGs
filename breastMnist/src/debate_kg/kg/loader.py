...
def serialize_triples_for_prompt(triples: list) -> str:
    """Serialize a subset of Triple objects into a compact text block for prompt injection.

    Used to build a focused KG from automatically retrieved triples, replacing the full KG
    in subsequent debate rounds.

    Args:
        triples: List of Triple objects to serialize.

    Returns:
        Multi-line string in the same format as serialize_kg_for_prompt.
    """
    lines: list[str] = [
        "KNOWLEDGE GRAPH (ACR BI-RADS Ultrasound Lexicon — retrieved)",
        "-" * 50,
    ]
